...
def pytest_configure(config: pytest.Config) -> None:
    ...
    logging.info(
        '\n==> pytest_configure 2("root"/conftest.py)'
    )  # <- not logged! TODO Why ?
    ...
    bdd_logger.log_hook(
        "pytest_configure"
    )  # TODO Fjerne her. Dette kan bdd_logger.configure gjøre
    ...
    ), f'Found value "{logger_config}" in TEST_CONTEXT logger_config'
    ...
    GLUE_LOGGER.warning(
        'This is a WARNING message from GLUE_LOGGER in "%s" module.', __name__
    )
...
#     bdd_logger.log_hook(
#         f"(test.name: {test})"
#     )  # TODO Fjerne her. Dette kan bdd_logger.before_test gjøre
...
#     bdd_logger.log_hook(
#         f"(test.name: {test})"
#     )  # TODO Fjerne her. Dette kan bdd_logger.after_test gjøre
...
def pytest_runtest_protocol(item: Item, nextitem: Optional[Item]) -> bool:
    ...
    )  # TODO Fjerne her. Dette kan bdd_logger gjøre
...
def pytest_bdd_before_scenario(
    ...
    # logging.info('%s', '--' * 50)
    logging.info(LogHelper.ret_func_name())
    LogHelper.log_func_name_with_info(feature.name, fillchar="H:")
    LogHelper.log_func_call()
    bdd_logger.log_hook(feature.name)  # TODO Fjerne her. Dette kan bdd_logger gjøre
...
def pytest_bdd_after_scenario(
    ...
    LogHelper.log_func_call()
    LogHelper.log_func_name_with_info(feature.name, fillchar="H:")
    bdd_logger.log_hook(scenario.name)  # TODO Fjerne her. Dette kan bdd_logger gjøre
...
def pytest_bdd_before_step(
    ...
    bdd_logger.log_hook(step.name)  # TODO Fjerne her. Dette kan bdd_logger gjøre
...
def pytest_bdd_after_step(
    ...
    bdd_logger.log_hook(
        step.name
    )  # TODO Fjerne her. Dette kan bdd_logger.after_step gjøre
...
def given_step_using_the_module(context: dict, module: str) -> None:
    ...
    assert (
        logger_name == KEY_LOG_GLUE
    ), f'The wanted logger is not in use! Using "{logger_name}"'
...
def given_file_uses_hooks_that_calls_corresponding_module_func(
    ...
    assert (
        file_name is not None or len(file_name) > 0
    ), 'A value for "file_name" was not supplied!'
...
def then_information_about_context_will_include(
    ...
    logging.info(
        'information about context ["%s"] %s include "%s"', info_in, will, info
    )
...
@then(
    parsers.parse('"{name}" should show that the {about} "{func_name}" have been run')
)
def then_shows_that_function_have_been_run(
    name: str, about: str, func_name: str
) -> None:
    assert about is not None, f'"{about}" is not a valid value!'
    assert len(about) > 0
    msg = f'*************> Then "{name}" should show that the {about} "{func_name}" have been run'
    logging.info(msg)
    ...
    if "hook-function" in about:
        ...
        ), f'(if hoock-func) ... didn\'t find "{KEY_HOOKS}" in TEST_CONTEXT!'
        ...
        assert (
            hooks.count("function") > 0
        ), f"Didn't find '{func_name}' in key \"{KEY_HOOKS}\" in TEST_CONTEXT!"
