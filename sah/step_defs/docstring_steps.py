...
@then(parse('I have a message:{dok_str}'))
def given_i_have_a_message_x(context, dok_str: str):
    ...
    log_glue(context=context, dok_str=dok_str)
    pprint(dok_str)
...
@given(parse('I have step with a Docstring:{doc_string}'))
...
@then(parse('I should be told it was {expected_num:d} line'))
@then(parse('I should be told it was {expected_num:d} lines'))
def then_i_should_be_told_it_was_1_line(context, expected_num: int):
    log_glue(context=context, expected_num=expected_num)
...
# @then(parse('Details related to incident will be returned:\n{content}'))
