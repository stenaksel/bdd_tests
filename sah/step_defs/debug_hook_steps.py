...
@given('this scenario is tagged with {str}')
@given(parse('this scenario is tagged with "{tag}"'))
def given_this_scenario_is_tagged_with_wip(tag: str):
    assert tag == 'wip'


@given('a {str} pytest.hook function in conftest.py')
@given(parse('a "{func_name}" pytest.hook function in conftest.py'))
def pytest_hook_function_in_conftest_py(context, func_name: str):
    ...
    logging.info(func_name)
    logging.info(func_name)
    assert func_name == 'pytest_bdd_before_scenario'   ##TODO remove
    called_functions = TEST_CONTEXT.get(KEY_FUNC, False)
    logging.info(called_functions)
    logging.info(called_functions)
    logging.info(called_functions)
    assert called_functions
    assert func_name in called_functions
...
def then_hook_function_execution_should_be_logged(context, func_name: str):
    ...
    log_glue(context=context, func_name=func_name)
    the_when_func_was_called = context.get('called:glue_func_no_params', False)
    assert the_when_func_was_called, 'The "glue_func_no_params" was not called!'
    # assert False
    log_glue_end(context)
