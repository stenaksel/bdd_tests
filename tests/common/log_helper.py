...
KEY_LOG_GLUE = (
    "log_glue"  # TODO: Values:False, True, Hooks (=True), Feature, Scenario, Step
)
...
class LogHelper:
    ...
    def log_dict_now(the_dict: dict, name: str = None, prefix: str = "\t\t") -> None:
        ...
        LogHelper.log_func_name(prev=1, informative=False)  # TODO delete one
    ...
    def log_func_call(
        ...
        calling_function = inspect.getframeinfo(frame).function
        ...
        wanted_logged = f"-> {calling_function}({args})"
    ...
    def log_func_name(prev: int = 0, informative=True) -> None:  # tested
        ...
        """
        caller = LogHelper.ret_func_name(prev + 2)  # TODO
        context = (
            "" if not informative else f"(prev={prev}) {LogHelper.ret_provider_info()}"
        )
        logging.info(
            "%s->%s%s%s",
            COL_INFO if not informative else COL_CONTEXT,
            COL_MSG,
            LogHelper.ret_func_name(0),
            context,
        )
        # logging.info(
        #     '%s->%s  %s(prev=%s) << %s %s',
    ...
    def log_func_call_info(
        info: str = "??", log_level: int = logging.INFO, prev: int = 0
    ) -> None:
        ...
        if caller.startswith("pytest_bdd_"):
            ...
            if "_before_" in caller:
                ...
                assert (
                    False
                ), f"Stopping in func: log_func_call_info, unhandled hook! ({caller})"
        ...
        if "==" in info:  # a hook
            ...
            logging.log(
                log_level, "|%s%-25s %s| %s", COL_CONTEXT, its_caller, info, caller
            )
        else:
            logging.log(
                log_level,
    ...
    @staticmethod
    def assert_messages(
        caplog, level, messages: List, in_sequence: bool = False
    ) -> None:
        print("#### messages:")
        for msg in messages:
            print(msg)
        ...
        filtered_records = [
            record for record in caplog.records if record.levelno == level
        ]
        messages_found = []
        lines: List[int] = []
        rest = messages.copy()
