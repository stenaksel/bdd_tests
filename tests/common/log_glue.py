...
def log_glue(**params) -> None:
    ...
    if ctx:
        ...
        # step_counter = ctx.get(KEY_STEP_COUNTER, 0)
        # step_counter = step_counter + 1
        # ctx[KEY_STEP_COUNTER] = ctx.get(KEY_STEP_COUNTER, 0) + 1
    ...
    if ctx is None:
        ...
        logging.debug('ctx2:\t%s', ret_dict_info('ctx:\t', ret_sorted(ctx)))

    if debug_params:
        # Then show each param below:
        for key, value in params.items():
            if isinstance(value, dict):
                param_info_ret += ret_dict_info(key, value, 'pd:\t')
            else:
    ...
    logging.info(' => %s context: %s)', glue_function, ret_dict_info('ctx', ret_sorted(ctx)))
    logging.debug('%s%s%s', COL_GLUE, '=' * (len(ret_func) + 2), COL_RESET)
...
def log_glue_end(ctx: dict = {}, print_prefix='<== ') -> None:
